...
test("pilot launch packet defaults to the latest rehearsal bundle", () => {
  ...
  assert.match(output, /pilot_launch_packet_source_bundle=.*20260622T030000Z-simulated-north-river/);
  const packet = fs.readFileSync(packetPath, "utf8");
  assert.match(packet, new RegExp(`Source bundle: \`.*${latestBundleDir.split(path.sep).pop()}\``));
});
