...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "club-content-pilot-real-onboarding-gaps-"));
...
test("real onboarding gap report groups missing real-world fields from a scaffolded worksheet", () => {
  const repoRoot = setupFixtureRepo();
  const scriptPath = path.join(repoRoot, "scripts", "pilot_real_onboarding_gaps.sh");
  ...
  try {
    ...
    assert.fail("expected gap report to fail for incomplete worksheet");
  } catch (error) {
    assert.equal(error.status, 1);
    output = String(error.stdout || "");
  }

  assert.match(output, /pilot_real_onboarding_gaps=NO_GO/);
  assert.match(output, /pilot_real_onboarding_next_step=fill_real_fields/);
  assert.match(output, /pilot_real_onboarding_gap_category=Candidate identity/);
  assert.match(output, /pilot_real_onboarding_gap_label=Candidate profile name/);
  assert.match(output, /pilot_real_onboarding_gap_category=People and ownership/);
  assert.match(output, /pilot_real_onboarding_gap_label=Executive sponsor/);
  assert.match(output, /pilot_real_onboarding_gap_category=Launch evidence/);
  assert.match(output, /pilot_real_onboarding_gap_label=Go-live owner signoff/);
  assert.match(output, /pilot_real_onboarding_gap_action=expected completed\/yes/);
});

test("real onboarding gap report passes when worksheet and launch evidence are complete", () => {
  const repoRoot = setupFixtureRepo();
  const scriptPath = path.join(repoRoot, "scripts", "pilot_real_onboarding_gaps.sh");
  const onboardingPath = path.join(repoRoot, "docs", "pilot-onboarding.md");
  ...
  assert.match(output, /pilot_real_onboarding_gaps=GO/);
  assert.match(output, /pilot_real_onboarding_gap_count=0/);
  assert.match(output, /pilot_real_onboarding_next_step=launch_from_onboarding/);
});
