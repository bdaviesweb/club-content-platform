...
echo ---
docker compose -f docker-compose.vps.yml exec -T postgres psql -U club -d club_content -c "select action, metadata->>'type' as type, metadata->'delivery'->>'mode' as mode, metadata->'delivery'->>'reason' as reason, created_at from audit_logs where entity_type='notification' order by created_at desc limit 5;"
INNER
