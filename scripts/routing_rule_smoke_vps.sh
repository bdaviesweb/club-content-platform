...
while (( SECONDS < deadline )); do
  ...
  if [[ -n "${row}" ]]; then
    IFS='|' read -r submission_id status approval_request_id approver_role routing_approver_role policy_source routing_source processing_error <<< "${row}"
    ...
    echo "Waiting for routing override. status=${status:-pending} approver_role=${approver_role:-pending} policy_source=${policy_source:-pending}"
  else
